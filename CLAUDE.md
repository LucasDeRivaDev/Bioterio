...
- **Sistema de confirmación de reposición de alimento en jaulas (22/05/2026):** `ModalReposicion` standalone con fecha/hora, tipo completa/parcial, kg (presets 1–8 kg), bioterios afectados, categorías opcionales y notas. `ModalCensoAlimento` muestra prompt "¿Se repuso alimento hoy?" con botones Sí/Parcial/No en días de alta probabilidad (lun/vie); si Sí/Parcial guarda reposición confirmada separada del censo. `consumoPorCenso`: campo `fuenteRelleno` (`'confirmado'` / `'declarado'` / `'ninguno'`); reposiciones confirmadas tienen prioridad total sobre `rellenoKg` declarado en censo. EWMA: peso ×1.4 para períodos con reposición confirmada + `bonusConfirmados = min(15, n×5)` puntos de confianza. Timeline: entradas de tipo `'reposicion'` con detalles de bioterios/categorías; badges de fuente ✅/⚠. LS key: `appMosca_alimento_reposiciones`.

- **Sistema de índice sanitario global y registro predictivo de incidentes (22/05/2026):**
  - `src/utils/sanitario.js`: constantes `CATEGORIAS` (5 categorías: sanitario, reproductivo, crías, ambiental, otro — cada una con tipos específicos), `SEVERIDADES` (leve/moderado/grave), `LISTA_BIOTERIOS`. Funciones: `calcularIndiceSanitario(camadas, incidentes, bioterioId)` → puntaje 0–100 con penalizaciones por graves (-8/c, máx -24), moderados (-4/c, máx -16), leves (-1/c, máx -5), fallos reproductivos (-5/c, máx -20), supervivencia baja (-15 o -7) y patrones críticos (-10/patrón). `nivelIndice(score)` → `{ label, emoji, color }` (🟢 Estable ≥80 / 🟡 Atención ≥50 / 🔴 Riesgo <50). `detectarPatrones(incidentes)` → agrupa por `tipo_incidente` en 90 días, alerta si ≥3 registros con ≥2 animales/camadas distintos o ≥4 totales; nivel `critico` si ≥5. `generarTendencias(incidentes, meses)` → datos mensuales con totales y tendencia %.
  - `src/pages/Incidentes.jsx` (reescritura completa): panel de índice sanitario 0–100 con desglose por colonia; sección de alertas de patrones repetitivos; 2 tabs (Lista / Estadísticas). KPIs: total, abiertos, graves, últimos 30d. BarChart de tendencias 6 meses con Recharts. Distribución por categoría (barras proporcionales). Filtros: colonia + categoría + severidad + estado (abierto/resuelto/todos). `ModalIncidente`: formulario con fecha, severidad (botones leve/moderado/grave), categoría (grid 5 botones), tipo (grid dinámico por categoría), descripción, animal asociado, camada asociada. Botones Resolver / Editar / Eliminar en cada fila.
  - `src/context/BiotheriumContext.jsx`: nuevos reducer case `EDITAR_INCIDENTE` y función `editarIncidente(datos)` (despacha + supabase update).
  - `src/pages/SelectorBioterio.jsx`: función `badgeSanitario(incidentes, bioterioId)` que clasifica en Estable/Atención/Riesgo según incidentes graves y moderados abiertos en últimos 90 días. Fetch ligero al montar: solo columnas `bioterio_id, severidad, resuelto, fecha` con filtro ≥90 días + `resuelto = false`. Badges visibles en card de Ratas (con texto) y en cada subgrupo de ratones (emoji + count).
  - **SQL requerida en Supabase:** `ALTER TABLE incidentes ADD COLUMN IF NOT EXISTS tipo_categoria text DEFAULT 'otro', ADD COLUMN IF NOT EXISTS tipo_incidente text DEFAULT 'otro', ADD COLUMN IF NOT EXISTS severidad text DEFAULT 'leve', ADD COLUMN IF NOT EXISTS animal_id uuid REFERENCES animales(id) ON DELETE SET NULL, ADD COLUMN IF NOT EXISTS camada_id uuid REFERENCES camadas(id) ON DELETE SET NULL, ADD COLUMN IF NOT EXISTS resuelto boolean DEFAULT false;`
...
- **Sistema unificado de planificación de colonia (24/05/2026):**
  ...
  - **Jerarquía de decisiones aplicada:** Supervivencia colonia > Mínimos reproductivos > Producción híbridos > Renovación > Calidad genética > Consanguinidad > Saturación > Sacrificios. Ninguna acción inferior puede romper una prioridad superior.
  - **Sin SQL nueva** — funciona con los datos ya existentes en Supabase.
